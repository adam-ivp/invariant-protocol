Go to your repo on GitHub, click README.md, click the pencil icon to edit, select all, delete everything, paste this:
...
Quickstart
npm install -g @ivp/cli
ivp spec validate examples/bridge_event_integrity.isl
ivp register --spec out/spec.json --protocol 0xYour --vault 0xVault --network base-sepolia
ivp watch --protocol 0xYourProtocol
Example output when a violation is detected:
=== IVP EPOCH RESULT ===
Epoch:      4821
Violated:   true

INVARIANT VIOLATION: swap_event_integrity
Constraint: emitted_amount_matches_balance_delta
event fromAmount:  1000   <- emitted by contract
balance_delta:      900   <- actual received (10% FoT tax)

Relayer overpays destination by 100 tokens per swap.
Coverage claim eligible. Proof window: 256 blocks.
Status: FINALIZED VIOLATED
See examples/ and demo/bridge-event-integrity-walkthrough.md.
...
Two functions with the same stated intent — one updates an enforcement bitmap, the other doesn't. Invisible at the call site. Surfaces in edge cases where the missing update silently bypasses the intended restriction. Active findings in this category across major lending deployments.
Incomplete fix propagation
A patch applied to one mirrored execution path but not its structural mirror. Ghost share pattern: rounding leaves total debt at zero while shares stay non-zero. The next borrower pays for it. Triggers through normal usage. No attack required. Active findings across CDP stablecoin protocols.
Event integrity divergence
A contract computes the correct value internally but emits a different value in the event log. Bridge relayers use emitted events as the source of truth for destination-side settlement. Active findings with PoCs verified on mainnet forks.
